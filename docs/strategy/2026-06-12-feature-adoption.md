...
- **Anthropic (own provider, lowest friction):** **strict tool-use schema** for agent outputs (the adapter already wraps `messages.stream` with `output_config`; structured output on the Anthropic path is the tool-schema / `tool_choice` pattern Spectra already uses for Pydantic validation — *not* an OpenAI-style `messages.parse`/`json_schema` call, so don't let an implementer drift toward that API) and the **`count_tokens` endpoint** (true tokenizer, retires the `tiktoken` under-count flagged in the architecture memo). Both low-friction on the Anthropic path. Plus a **Haiku/Sonnet/Opus tier mix** — reserve Opus for high-signal dimensions — the cheap half of the future Cost-Governor.
