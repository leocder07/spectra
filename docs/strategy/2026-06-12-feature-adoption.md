...
## ✅ Genuinely free / now — the only items that survive the freeze

These are optimizations or presentations of *already-shipped* behavior — no new surface, no new dependency, no new buyer required.

1. **Adopt the OpenAI Agents-SDK span taxonomy** (`generation` / `handoff` / `guardrail` / `function`) as the naming convention for Spectra's **existing** OTel spans across Stages 2–5. Spectra is already OTel-native (vendor-neutral, which the SDK is not), so this is a rename/convention pass on shipped tracing. Makes the signed provenance trail uniformly queryable. *(the cleanest pure win)*
2. **Render the per-agent / per-stage / cache-savings cost+token breakdown** in the Rich CLI summary and the Stage-6 report. The data already exists (`CostTrackerPort.cost_usd`, `AnalysisReport.cost_saved_usd`). Pure presentation — and it lets the signed receipt carry a tamper-evident cost-of-audit attestation. *(OpenHands metrics pattern)*
3. **Verify-then-maybe-skip:** assert prefix-stable prompt layout in the existing cache smoke test, and confirm the `effort` level is already in the composite cache key + receipt. Both are likely **already done** (prompt caching + per-role effort overrides are shipped) — check before spending an hour.
...
- **Anthropic (own provider, lowest friction):** native **structured outputs** (`output_config.format` json_schema + strict tool-use via `messages.parse`) and the **`count_tokens` endpoint** (true tokenizer, retires the `tiktoken` under-count flagged in the architecture memo). Both `now-free`-ish on the Anthropic path. Plus a **Haiku/Sonnet/Opus tier mix** — reserve Opus for high-signal dimensions — which is the cheap half of the future Cost-Governor.
...
- **Now (free):** items 1–4 above.
- **Pre-launch, only if a trust number is needed in a conversation:** sign the existing catch-rate (#4). Do **not** author the F1 corpus speculatively.
...
The freeze holds. The only work to do *now* is two clean free wins (span-taxonomy rename, cost-breakdown render) and, if a conversation needs it, signing the catch-rate number Spectra already computes. Everything genuinely new — the eval harness and the repo-map, however transformative — waits for the demand signal that names it, and is then built as the cheapest thing that answers that signal, re-evaluated against the then-current provider APIs. The durable insight is the adoption rule itself, now encoded as a lens in the reusable [`project-audit`](AUDIT-PROMPT.md) workflow: *borrow the technique, sign the artifact.*
